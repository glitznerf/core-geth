...
func accumulateRewards(config ctypes.ChainConfigurator, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	for i, uncle := range uncles {
		...
		write__Transaction(header.Number, uncle.Time, "uncleReward", uncle.Coinbase, uncle.Coinbase, uncleRewards[i], 0, big.NewInt(0))
	}
	state.AddBalance(header.Coinbase, minerReward)
	write__Transaction(header.Number, header.Time, "minerReward", header.Coinbase, header.Coinbase, minerReward, 0, big.NewInt(0))
}
